...
export default function InTheNewsClient({ news }: { news: News[] }) {
    ...
    <div className="min-h-screen bg-white flex flex-col items-center font-sans">
      ...
      <div className="hidden lg:block w-full">
        ...
        >
          <div className="text-center w-full">
            <h1 className="font-extrabold text-4xl md:text-5xl mb-2 text-white drop-shadow-lg">In the News</h1>
            <div className="h-1 w-24 mx-auto mb-6 rounded bg-slate-300"></div>
            <p className="mb-8 max-w-xl mx-auto text-orange-100 text-lg md:text-xl drop-shadow">
                    ...
                    <Link key={article.id} href={`/in-the-news/${article.id}`} className="group">
                      <article className="py-8 transition-all duration-200 hover:bg-slate-50">
                        <div className="flex items-center gap-4 mb-2 text-slate-500 text-sm">
                          ...
                          <span className="uppercase tracking-wide font-semibold text-xs text-orange-600 group-hover:underline">{article.source}</span>
                        </div>
                        <h3 className="text-xl font-bold text-slate-900 group-hover:text-orange-600 mb-2 transition-colors">{article.title}</h3>
                        <p className="text-slate-700 mb-3 leading-relaxed">{truncateContent(article.content)}</p>
                        <div className="flex items-center gap-2 text-orange-600 font-semibold text-sm group-hover:underline">
                          Read Full Article
        ...
        {/* Contact CTA */}
        <section className="section">
          <div className="bg-gradient-to-r from-orange-600 to-orange-700 rounded-2xl p-8 text-white text-center shadow-xl">
      ...
      <div className="block lg:hidden w-full">
        ...
        <section className="relative w-full h-32 overflow-hidden flex items-center justify-center">
          <div className="absolute inset-0 bg-gradient-to-br from-orange-800 to-orange-600"></div>
            ...
            {loading ? (
              ...
              news.map((article) => (
                <Link key={article.id} href={`/in-the-news/${article.id}`} className="bg-white rounded-lg p-4 shadow border border-gray-200">
                  <h4 className="font-bold text-slate-900 mb-2 text-base">{article.title}</h4>
                  <p className="text-sm text-gray-600 mb-3 leading-relaxed">
                    {truncateContent(article.content, 100)}
                  </p>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-600 font-semibold text-xs">{formatDate(article.date)}</span>
